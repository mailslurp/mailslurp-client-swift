...
```swift
    open class func getExpirationDefaults(completion: @escaping (_ data: ExpirationDefaults?, _ error: Error?) -> Void)
```
...
// Get default expiration settings
ExpiredControllerAPI.getExpirationDefaults() { (response, error) in
...
# **getExpiredInboxes**
```swift
    open class func getExpiredInboxes(page: Int? = nil, size: Int? = nil, sort: Sort_getExpiredInboxes? = nil, since: Date? = nil, before: Date? = nil, completion: @escaping (_ data: PageExpiredInboxRecordProjection?, _ error: Error?) -> Void)
```
...
// List records of expired inboxes
ExpiredControllerAPI.getExpiredInboxes(page: page, size: size, sort: sort, since: since, before: before) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
