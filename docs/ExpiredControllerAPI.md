...
# **getExpiredInboxes**
```swift
    open class func getExpiredInboxes( before: Date? = nil,  page: Int? = nil,  since: Date? = nil,  size: Int? = nil,  sort: Sort_getExpiredInboxes? = nil) -> Promise<PageExpiredInboxRecordProjection>
```
...
// List records of expired inboxes
ExpiredControllerAPI.getExpiredInboxes(before: before, page: page, since: since, size: size, sort: sort).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
