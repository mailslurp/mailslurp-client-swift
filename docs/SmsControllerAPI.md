...
# **deleteSmsMessage**
```swift
    open class func deleteSmsMessage(smsId: UUID, completion: @escaping (_ data: Void?, _ error: Error?) -> Void)
```
...
// Delete SMS message.
SmsControllerAPI.deleteSmsMessage(smsId: smsId) { (response, error) in
...
# **deleteSmsMessages**
```swift
    open class func deleteSmsMessages(phoneNumberId: UUID? = nil, completion: @escaping (_ data: Void?, _ error: Error?) -> Void)
```
...
// Delete all SMS messages
SmsControllerAPI.deleteSmsMessages(phoneNumberId: phoneNumberId) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
# **getSmsMessage**
```swift
    open class func getSmsMessage(smsId: UUID, completion: @escaping (_ data: SmsMessage?, _ error: Error?) -> Void)
```
...
// Get SMS content including body. Expects SMS to exist by ID. For SMS that may not have arrived yet use the WaitForController.
SmsControllerAPI.getSmsMessage(smsId: smsId) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
```swift
    open class func getSmsMessagesPaginated(phoneNumber: UUID? = nil, page: Int? = nil, size: Int? = nil, sort: Sort_getSmsMessagesPaginated? = nil, unreadOnly: Bool? = nil, since: Date? = nil, before: Date? = nil, completion: @escaping (_ data: PageSmsProjection?, _ error: Error?) -> Void)
```
...
// Get all SMS messages in all phone numbers in paginated form. .
SmsControllerAPI.getSmsMessagesPaginated(phoneNumber: phoneNumber, page: page, size: size, sort: sort, unreadOnly: unreadOnly, since: since, before: before) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
