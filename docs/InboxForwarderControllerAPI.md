...
```swift
    open class func getAllInboxForwarderEvents( page: Int? = nil,  size: Int? = nil,  sort: Sort_getAllInboxForwarderEvents? = nil) -> Promise<PageInboxForwarderEvents>
```
...
// Get all inbox forwarder events
InboxForwarderControllerAPI.getAllInboxForwarderEvents(page: page, size: size, sort: sort).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
