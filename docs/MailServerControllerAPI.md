...
let dNSLookupsOptions = DNSLookupsOptions(lookups: [DNSLookupOptions(hostname: "hostname_example", recordTypes: ["recordTypes_example"], omitFinalDNSDot: false)]) // DNSLookupsOptions | 

// Lookup DNS records for multiple domains
MailServerControllerAPI.getDnsLookups(dNSLookupsOptions: dNSLookupsOptions).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
