...
# **getPhoneNumbers**
```swift
    open class func getPhoneNumbers( page: Int? = nil,  size: Int? = nil,  sort: Sort_getPhoneNumbers? = nil,  since: Date? = nil,  before: Date? = nil) -> Promise<PagePhoneNumberProjection>
```
...
PhoneControllerAPI.getPhoneNumbers(page: page, size: size, sort: sort, since: since, before: before).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
...
```swift
    open class func testPhoneNumberSendSms( phoneNumberId: UUID,  testPhoneNumberOptions: TestPhoneNumberOptions) -> Promise<Void>
```
...
PhoneControllerAPI.testPhoneNumberSendSms(phoneNumberId: phoneNumberId, testPhoneNumberOptions: testPhoneNumberOptions).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
