...
# **getAttachments**
```swift
    open class func getAttachments(page: Int? = nil, size: Int? = nil, sort: Sort_getAttachments? = nil, completion: @escaping (_ data: PageAttachmentEntity?, _ error: Error?) -> Void)
```
...
// Get email attachments
AttachmentControllerAPI.getAttachments(page: page, size: size, sort: sort) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
