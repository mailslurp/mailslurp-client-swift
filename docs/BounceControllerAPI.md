...
# **getReputationItems**
```swift
    open class func getReputationItems( page: Int? = nil,  size: Int? = nil,  sort: Sort_getReputationItems? = nil,  since: Date? = nil,  before: Date? = nil) -> Promise<PageReputationItems>
```
...
// Get paginated list of reputation items.
BounceControllerAPI.getReputationItems(page: page, size: size, sort: sort, since: since, before: before).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
