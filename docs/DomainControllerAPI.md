...
# **updateDomain**
```swift
    open class func updateDomain(_id: UUID, updateDomainOptions: UpdateDomainOptions, completion: @escaping (_ data: DomainDto?, _ error: Error?) -> Void)
```
...
// Update a domain
DomainControllerAPI.updateDomain(_id: _id, updateDomainOptions: updateDomainOptions) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
