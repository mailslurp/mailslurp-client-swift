...
# **downloadAttachment**
```swift
    open class func downloadAttachment( attachmentId: String,  emailId: UUID,  apiKey: String? = nil) -> Promise<Data>
```
...
// Get email attachment bytes. Returned as `octet-stream` with content type header. If you have trouble with byte responses try the `downloadAttachmentBase64` response endpoints and convert the base 64 encoded content to a file or string.
EmailControllerAPI.downloadAttachment(attachmentId: attachmentId, emailId: emailId, apiKey: apiKey).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
...
```swift
    open class func downloadAttachmentBase64( attachmentId: String,  emailId: UUID) -> Promise<DownloadAttachmentDto>
```
...
// Get email attachment as base64 encoded string as an alternative to binary responses. Decode the `base64FileContents` as a `utf-8` encoded string or array of bytes depending on the `contentType`.
EmailControllerAPI.downloadAttachmentBase64(attachmentId: attachmentId, emailId: emailId).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
...
```swift
    open class func getAttachmentMetaData( attachmentId: String,  emailId: UUID) -> Promise<AttachmentMetaData>
```
...
// Get email attachment metadata. This is the `contentType` and `contentLength` of an attachment. To get the individual attachments  use the `downloadAttachment` methods.
EmailControllerAPI.getAttachmentMetaData(attachmentId: attachmentId, emailId: emailId).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
...
# **getAttachments1**
```swift
    open class func getAttachments1( emailId: UUID) -> Promise<[AttachmentMetaData]>
```
...
// Get all email attachment metadata. Metadata includes name and size of attachments.
EmailControllerAPI.getAttachments1(emailId: emailId).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
...
# **getEmailHTMLQuery**
```swift
    open class func getEmailHTMLQuery( emailId: UUID,  htmlSelector: String? = nil) -> Promise<EmailTextLinesResult>
```
...
# **getEmailsPaginated**
```swift
    open class func getEmailsPaginated( before: Date? = nil,  inboxId: [UUID]? = nil,  page: Int? = nil,  searchFilter: String? = nil,  since: Date? = nil,  size: Int? = nil,  sort: Sort_getEmailsPaginated? = nil,  unreadOnly: Bool? = nil) -> Promise<PageEmailProjection>
```
...
// Get all emails in all inboxes in paginated form. Email API list all.
EmailControllerAPI.getEmailsPaginated(before: before, inboxId: inboxId, page: page, searchFilter: searchFilter, since: since, size: size, sort: sort, unreadOnly: unreadOnly).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
...
let emailAddress = "emailAddress_example" // String | emailAddress
let size = "size_example" // String | size (optional)
...
```swift
    open class func getLatestEmailInInbox( inboxId: UUID? = nil) -> Promise<Email>
```
...
```swift
    open class func getOrganizationEmailsPaginated( before: Date? = nil,  inboxId: [UUID]? = nil,  page: Int? = nil,  searchFilter: String? = nil,  since: Date? = nil,  size: Int? = nil,  sort: Sort_getOrganizationEmailsPaginated? = nil,  unreadOnly: Bool? = nil) -> Promise<PageEmailProjection>
```
...
// Get all organization emails. List team or shared test email accounts
EmailControllerAPI.getOrganizationEmailsPaginated(before: before, inboxId: inboxId, page: page, searchFilter: searchFilter, since: since, size: size, sort: sort, unreadOnly: unreadOnly).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
...
let replyToEmailOptions = ReplyToEmailOptions(attachments: ["attachments_example"], body: "body_example", charset: "charset_example", from: "from_example", isHTML: false, replyTo: "replyTo_example", sendStrategy: "sendStrategy_example", template: 123, templateVariables: "TODO", useInboxName: false) // ReplyToEmailOptions | replyToEmailOptions
...
```swift
    open class func sendEmailSourceOptional( inboxId: UUID? = nil,  useDomainPool: Bool? = nil,  sendEmailOptions: SendEmailOptions? = nil) -> Promise<Void>
```
...
// Send email
EmailControllerAPI.sendEmailSourceOptional(inboxId: inboxId, useDomainPool: useDomainPool, sendEmailOptions: sendEmailOptions).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
