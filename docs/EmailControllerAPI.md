...
# **replyToEmail1**
```swift
    open class func replyToEmail1(emailId: UUID, replyToEmailOptions: ReplyToEmailOptions, completion: @escaping (_ data: SentEmailDto?, _ error: Error?) -> Void)
```
...
// Reply to an email
EmailControllerAPI.replyToEmail1(emailId: emailId, replyToEmailOptions: replyToEmailOptions) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
