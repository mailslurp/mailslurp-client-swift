...
# **getEmailTextLines**
```swift
    open class func getEmailTextLines(emailId: UUID, decodeHtmlEntities: Bool? = nil, completion: @escaping (_ data: EmailTextLinesResult?, _ error: Error?) -> Void)
```
...
// Parse and return text from an email, stripping HTML and decoding encoded characters
EmailControllerAPI.getEmailTextLines(emailId: emailId, decodeHtmlEntities: decodeHtmlEntities) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
