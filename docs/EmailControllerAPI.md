...
# **getEmailTextLines**
```swift
    open class func getEmailTextLines(decodeHtmlEntities: Bool, emailId: UUID, completion: @escaping (_ data: EmailTextLinesResult?, _ error: Error?) -> Void)
```
...
let decodeHtmlEntities = true // Bool | Decode HTML entities
let emailId = 987 // UUID | ID of email to fetch text for

// Parse and return text from an email, stripping HTML and decoding encoded characters
EmailControllerAPI.getEmailTextLines(decodeHtmlEntities: decodeHtmlEntities, emailId: emailId) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
