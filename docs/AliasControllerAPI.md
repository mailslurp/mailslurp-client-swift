...
```swift
    open class func getAliasVerification(aliasId: UUID, emailAddress: String, verificationToken: String, completion: @escaping (_ data: AliasVerificationResult?, _ error: Error?) -> Void)
```
...
let verificationToken = "verificationToken_example" // String | verificationToken

// Get validation result from alias verification
AliasControllerAPI.getAliasVerification(aliasId: aliasId, emailAddress: emailAddress, verificationToken: verificationToken) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
