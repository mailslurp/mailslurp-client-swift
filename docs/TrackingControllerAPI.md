...
```swift
    open class func getAllTrackingPixels( page: Int? = nil,  size: Int? = nil,  sort: Sort_getAllTrackingPixels? = nil) -> Promise<PageTrackingPixelProjection>
```
...
// Get tracking pixels
TrackingControllerAPI.getAllTrackingPixels(page: page, size: size, sort: sort).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
