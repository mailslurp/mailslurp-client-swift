...
# **getTrackingPixel**
```swift
    open class func getTrackingPixel(id: UUID, completion: @escaping (_ data: TrackingPixelDto?, _ error: Error?) -> Void)
```
...
// Get pixel
TrackingControllerAPI.getTrackingPixel(id: id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
