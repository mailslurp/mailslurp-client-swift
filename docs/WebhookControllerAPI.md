...
```swift
    open class func getTestWebhookPayload(completion: @escaping (_ data: WebhookPayload?, _ error: Error?) -> Void)
```
...
// Get test webhook payload example
WebhookControllerAPI.getTestWebhookPayload() { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
