...
@objc open class CommonActionsControllerAPI : NSObject {
     ...
     */
    open class func createNewEmailAddress(apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: Inbox?,_ error: Error?) -> Void)) {
        createNewEmailAddressWithRequestBuilder().execute(apiResponseQueue) { result -> Void in
            switch result {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func createNewEmailAddress1(apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: Inbox?,_ error: Error?) -> Void)) {
        createNewEmailAddress1WithRequestBuilder().execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
