...
@objc open class MissedEmailControllerAPI : NSObject {
    ...
    /**
     ...
     - parameter inboxId: (path) Optional inbox ID filter 
     - parameter timeout: (path) Optional timeout milliseconds 
     ...
     - returns: Promise<MissedEmail>
     */
    open class func waitForNthMissedEmail( inboxId: UUID,  timeout: Int64,  index: Int? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<MissedEmail> {
        let deferred = Promise<MissedEmail>.pending()
        waitForNthMissedEmailWithRequestBuilder(inboxId: inboxId, timeout: timeout, index: index).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<MissedEmail> 
     */
    open class func waitForNthMissedEmailWithRequestBuilder(inboxId: UUID, timeout: Int64, index: Int? = nil) -> RequestBuilder<MissedEmail> {
        var path = "/missed-emails/waitForNthMissedEmail"
        ...
        let timeoutPreEscape = "\(APIHelper.mapValueToPathItem(timeout))"
        let timeoutPostEscape = timeoutPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{timeout}", with: timeoutPostEscape, options: .literal, range: nil)
        let URLString = mailslurpAPI.basePath + path
        let parameters: [String: Any]? = nil
