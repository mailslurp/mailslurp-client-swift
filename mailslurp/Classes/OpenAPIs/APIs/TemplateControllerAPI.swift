...
@objc open class TemplateControllerAPI : NSObject {
     ...
     */
    open class func getAllTemplates( before: Date? = nil,  page: Int? = nil,  since: Date? = nil,  size: Int? = nil,  sort: Sort_getAllTemplates? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<PageTemplateProjection> {
        let deferred = Promise<PageTemplateProjection>.pending()
        getAllTemplatesWithRequestBuilder(before: before, page: page, since: since, size: size, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
     ...
     */
    open class func getAllTemplatesWithRequestBuilder(before: Date? = nil, page: Int? = nil, since: Date? = nil, size: Int? = nil, sort: Sort_getAllTemplates? = nil) -> RequestBuilder<PageTemplateProjection> {
