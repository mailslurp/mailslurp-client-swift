...
@objc open class TrackingControllerAPI : NSObject {
     ...
     */
    open class func getAllTrackingPixels( page: Int? = nil,  searchFilter: String? = nil,  size: Int? = nil,  sort: Sort_getAllTrackingPixels? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<PageTrackingPixelProjection> {
        let deferred = Promise<PageTrackingPixelProjection>.pending()
        getAllTrackingPixelsWithRequestBuilder(page: page, searchFilter: searchFilter, size: size, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
     ...
     */
    open class func getAllTrackingPixelsWithRequestBuilder(page: Int? = nil, searchFilter: String? = nil, size: Int? = nil, sort: Sort_getAllTrackingPixels? = nil) -> RequestBuilder<PageTrackingPixelProjection> {
        let path = "/tracking/pixels"
        let URLString = mailslurpAPI.basePath + path
