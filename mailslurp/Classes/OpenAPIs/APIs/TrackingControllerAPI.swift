...
@objc open class TrackingControllerAPI : NSObject {
     ...
     */
    open class func getAllTrackingPixels( before: Date? = nil,  page: Int? = nil,  searchFilter: String? = nil,  since: Date? = nil,  size: Int? = nil,  sort: Sort_getAllTrackingPixels? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<PageTrackingPixelProjection> {
        let deferred = Promise<PageTrackingPixelProjection>.pending()
        getAllTrackingPixelsWithRequestBuilder(before: before, page: page, searchFilter: searchFilter, since: since, size: size, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
     ...
     */
    open class func getAllTrackingPixelsWithRequestBuilder(before: Date? = nil, page: Int? = nil, searchFilter: String? = nil, since: Date? = nil, size: Int? = nil, sort: Sort_getAllTrackingPixels? = nil) -> RequestBuilder<PageTrackingPixelProjection> {
        let path = "/tracking/pixels"
        let URLString = mailslurpAPI.basePath + path
