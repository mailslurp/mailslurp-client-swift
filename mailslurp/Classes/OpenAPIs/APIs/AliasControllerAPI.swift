...
@objc open class AliasControllerAPI : NSObject {
    ...
    /**
     ...
     - parameter verificationToken: (query) verificationToken 
     - parameter apiResponseQueue: The queue on which api response is dispatched.
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getAliasVerification(aliasId: UUID, emailAddress: String, verificationToken: String, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: AliasVerificationResult?,_ error: Error?) -> Void)) {
        getAliasVerificationWithRequestBuilder(aliasId: aliasId, emailAddress: emailAddress, verificationToken: verificationToken).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     Get validation result from alias verification
     - GET /aliases/{aliasId}/verification
     ...
     - parameter emailAddress: (query) emailAddress 
     - parameter verificationToken: (query) verificationToken 
     - returns: RequestBuilder<AliasVerificationResult> 
     */
    open class func getAliasVerificationWithRequestBuilder(aliasId: UUID, emailAddress: String, verificationToken: String) -> RequestBuilder<AliasVerificationResult> {
        var path = "/aliases/{aliasId}/verification"
        let aliasIdPreEscape = "\(APIHelper.mapValueToPathItem(aliasId))"
        let aliasIdPostEscape = aliasIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{aliasId}", with: aliasIdPostEscape, options: .literal, range: nil)
        let URLString = mailslurpAPI.basePath + path
        ...
        url?.queryItems = APIHelper.mapValuesToQueryItems([
            ...
            "verificationToken": verificationToken.encodeToJSON()
        ])

        let requestBuilder: RequestBuilder<AliasVerificationResult>.Type = mailslurpAPI.requestBuilderFactory.getBuilder()
