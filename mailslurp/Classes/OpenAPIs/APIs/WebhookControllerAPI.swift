...
@objc open class WebhookControllerAPI : NSObject {
     ...
     */
    open class func getWebhookResultsUnseenErrorCount( inboxId: UUID, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<UnseenErrorCountDto> {
        let deferred = Promise<UnseenErrorCountDto>.pending()
        getWebhookResultsUnseenErrorCountWithRequestBuilder(inboxId: inboxId).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
     ...
     */
    open class func getWebhookResultsUnseenErrorCountWithRequestBuilder(inboxId: UUID) -> RequestBuilder<UnseenErrorCountDto> {
        var path = "/webhooks/results/unseen-count"
        let inboxIdPreEscape = "\(APIHelper.mapValueToPathItem(inboxId))"
        let inboxIdPostEscape = inboxIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{inboxId}", with: inboxIdPostEscape, options: .literal, range: nil)
        let URLString = mailslurpAPI.basePath + path
