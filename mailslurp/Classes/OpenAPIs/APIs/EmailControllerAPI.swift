...
@objc open class EmailControllerAPI : NSObject {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func replyToEmail1(emailId: UUID, replyToEmailOptions: ReplyToEmailOptions, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: SentEmailDto?,_ error: Error?) -> Void)) {
        replyToEmail1WithRequestBuilder(emailId: emailId, replyToEmailOptions: replyToEmailOptions).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
     ...
     */
    open class func replyToEmail1WithRequestBuilder(emailId: UUID, replyToEmailOptions: ReplyToEmailOptions) -> RequestBuilder<SentEmailDto> {
        var path = "/emails/{emailId}"
        let emailIdPreEscape = "\(APIHelper.mapValueToPathItem(emailId))"
        let emailIdPostEscape = emailIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{emailId}", with: emailIdPostEscape, options: .literal, range: nil)
