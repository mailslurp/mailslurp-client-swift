...
@objc open class EmailControllerAPI : NSObject {
    ...
    /**
     ...
     - parameter htmlSelector: (path) HTML selector to search for. Uses JQuery/JSoup/CSS style selector like &#39;.my-div&#39; to match content. See https://jsoup.org/apidocs/org/jsoup/select/Selector.html for more information. 
     - parameter apiResponseQueue: The queue on which api response is dispatched.
     - parameter completion: completion handler to receive the data and the error objects
     */
    ...
    /**
     Parse and return text from an email, stripping HTML and decoding encoded characters
     - POST /emails/{emailId}/html-query
    ...
    open class func getEmailHTMLQueryWithRequestBuilder(emailId: UUID, htmlSelector: String) -> RequestBuilder<EmailTextLinesResult> {
        var path = "/emails/{emailId}/html-query"
        let emailIdPreEscape = "\(APIHelper.mapValueToPathItem(emailId))"
        let emailIdPostEscape = emailIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{emailId}", with: emailIdPostEscape, options: .literal, range: nil)
        let htmlSelectorPreEscape = "\(APIHelper.mapValueToPathItem(htmlSelector))"
        let htmlSelectorPostEscape = htmlSelectorPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{htmlSelector}", with: htmlSelectorPostEscape, options: .literal, range: nil)
        let URLString = mailslurpAPI.basePath + path
        let parameters: [String:Any]? = nil
    ...
    /**
     ...
     - parameter decodeHtmlEntities: (path) Decode HTML entities 
     - parameter emailId: (path) ID of email to fetch text for 
     - parameter apiResponseQueue: The queue on which api response is dispatched.
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getEmailTextLines(decodeHtmlEntities: Bool, emailId: UUID, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: EmailTextLinesResult?,_ error: Error?) -> Void)) {
        getEmailTextLinesWithRequestBuilder(decodeHtmlEntities: decodeHtmlEntities, emailId: emailId).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<EmailTextLinesResult> 
     */
    open class func getEmailTextLinesWithRequestBuilder(decodeHtmlEntities: Bool, emailId: UUID) -> RequestBuilder<EmailTextLinesResult> {
        var path = "/emails/{emailId}/text"
        let decodeHtmlEntitiesPreEscape = "\(APIHelper.mapValueToPathItem(decodeHtmlEntities))"
        let decodeHtmlEntitiesPostEscape = decodeHtmlEntitiesPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{decodeHtmlEntities}", with: decodeHtmlEntitiesPostEscape, options: .literal, range: nil)
        let emailIdPreEscape = "\(APIHelper.mapValueToPathItem(emailId))"
        let emailIdPostEscape = emailIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{emailId}", with: emailIdPostEscape, options: .literal, range: nil)
        let URLString = mailslurpAPI.basePath + path
