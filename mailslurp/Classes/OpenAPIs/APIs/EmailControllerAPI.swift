...
@objc open class EmailControllerAPI : NSObject {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getEmailTextLines(emailId: UUID, decodeHtmlEntities: Bool? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: EmailTextLinesResult?,_ error: Error?) -> Void)) {
        getEmailTextLinesWithRequestBuilder(emailId: emailId, decodeHtmlEntities: decodeHtmlEntities).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<EmailTextLinesResult> 
     */
    open class func getEmailTextLinesWithRequestBuilder(emailId: UUID, decodeHtmlEntities: Bool? = nil) -> RequestBuilder<EmailTextLinesResult> {
