...
@objc open class ExpiredControllerAPI : NSObject {
     ...
     */
    open class func getExpiredInboxes( before: Date? = nil,  page: Int? = nil,  since: Date? = nil,  size: Int? = nil,  sort: Sort_getExpiredInboxes? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<PageExpiredInboxRecordProjection> {
        let deferred = Promise<PageExpiredInboxRecordProjection>.pending()
        getExpiredInboxesWithRequestBuilder(before: before, page: page, since: since, size: size, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
     ...
     */
    open class func getExpiredInboxesWithRequestBuilder(before: Date? = nil, page: Int? = nil, since: Date? = nil, size: Int? = nil, sort: Sort_getExpiredInboxes? = nil) -> RequestBuilder<PageExpiredInboxRecordProjection> {
        let path = "/expired"
        let URLString = mailslurpAPI.basePath + path
        let parameters: [String: Any]? = nil
