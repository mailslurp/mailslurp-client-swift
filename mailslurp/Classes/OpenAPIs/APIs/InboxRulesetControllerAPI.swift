...
@objc open class InboxRulesetControllerAPI : NSObject {
     ...
     */
    open class func getInboxRulesets( inboxId: UUID? = nil,  page: Int? = nil,  searchFilter: String? = nil,  size: Int? = nil,  sort: Sort_getInboxRulesets? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<PageInboxRulesetDto> {
        let deferred = Promise<PageInboxRulesetDto>.pending()
        getInboxRulesetsWithRequestBuilder(inboxId: inboxId, page: page, searchFilter: searchFilter, size: size, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
     ...
     */
    open class func getInboxRulesetsWithRequestBuilder(inboxId: UUID? = nil, page: Int? = nil, searchFilter: String? = nil, size: Int? = nil, sort: Sort_getInboxRulesets? = nil) -> RequestBuilder<PageInboxRulesetDto> {
        let path = "/rulesets"
        let URLString = mailslurpAPI.basePath + path
        let parameters: [String: Any]? = nil
