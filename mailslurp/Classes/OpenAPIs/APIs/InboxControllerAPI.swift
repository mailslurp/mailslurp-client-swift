...
@objc open class InboxControllerAPI : NSObject {
    ...
    /**
     ...
     - returns: Promise<Void>
     */
    open class func listInboxRulesets( inboxId: UUID,  page: Int? = nil,  size: Int? = nil,  sort: Sort_listInboxRulesets? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<Void> {
        let deferred = Promise<Void>.pending()
        listInboxRulesetsWithRequestBuilder(inboxId: inboxId, page: page, size: size, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case .success:
                deferred.resolver.fulfill(())
            case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    open class func listInboxRulesetsWithRequestBuilder(inboxId: UUID, page: Int? = nil, size: Int? = nil, sort: Sort_listInboxRulesets? = nil) -> RequestBuilder<Void> {
        var path = "/inboxes/{inboxId}/rulesets"
        let inboxIdPreEscape = "\(APIHelper.mapValueToPathItem(inboxId))"
        let inboxIdPostEscape = inboxIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{inboxId}", with: inboxIdPostEscape, options: .literal, range: nil)
