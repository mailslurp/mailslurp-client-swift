...
@objc open class InboxControllerAPI : NSObject {
     ...
     */
    open class func createInbox(_description: String? = nil, emailAddress: String? = nil, expiresAt: Date? = nil, favourite: Bool? = nil, name: String? = nil, tags: [String]? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: Inbox?,_ error: Error?) -> Void)) {
        createInboxWithRequestBuilder(_description: _description, emailAddress: emailAddress, expiresAt: expiresAt, favourite: favourite, name: name, tags: tags).execute(apiResponseQueue) { result -> Void in
            switch result {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getEmails(inboxId: UUID, limit: Int? = nil, minCount: Int64? = nil, retryTimeout: Int64? = nil, since: Date? = nil, sort: Sort_getEmails? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: [EmailPreview]?,_ error: Error?) -> Void)) {
        getEmailsWithRequestBuilder(inboxId: inboxId, limit: limit, minCount: minCount, retryTimeout: retryTimeout, since: since, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getInboxes(apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: [Inbox]?,_ error: Error?) -> Void)) {
        getInboxesWithRequestBuilder().execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func sendEmailAndConfirm(inboxId: UUID, sendEmailOptions: SendEmailOptions? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: SentEmail?,_ error: Error?) -> Void)) {
    ...
    /**
     ...
     - parameter inboxId: (path) ID of the inbox you want to send the email from 
     - parameter sendEmailOptions: (body) Options for the email (optional)
     - returns: RequestBuilder<SentEmail> 
     */
    open class func sendEmailAndConfirmWithRequestBuilder(inboxId: UUID, sendEmailOptions: SendEmailOptions? = nil) -> RequestBuilder<SentEmail> {
        ...
        let requestBuilder: RequestBuilder<SentEmail>.Type = mailslurpAPI.requestBuilderFactory.getBuilder()
