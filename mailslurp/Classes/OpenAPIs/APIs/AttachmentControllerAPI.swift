...
@objc open class AttachmentControllerAPI : NSObject {
     ...
     */
    public enum Sort_getAttachments1: String, CaseIterable {
        case asc = "ASC"
        case desc = "DESC"
    }
     ...
     */
    open class func getAttachments1( page: Int? = nil,  size: Int? = nil,  sort: Sort_getAttachments1? = nil,  fileNameFilter: String? = nil,  since: Date? = nil,  before: Date? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<PageAttachmentEntity> {
        let deferred = Promise<PageAttachmentEntity>.pending()
        getAttachments1WithRequestBuilder(page: page, size: size, sort: sort, fileNameFilter: fileNameFilter, since: since, before: before).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
     ...
     */
    open class func getAttachments1WithRequestBuilder(page: Int? = nil, size: Int? = nil, sort: Sort_getAttachments1? = nil, fileNameFilter: String? = nil, since: Date? = nil, before: Date? = nil) -> RequestBuilder<PageAttachmentEntity> {
        let path = "/attachments"
        let URLString = mailslurpAPI.basePath + path
        let parameters: [String: Any]? = nil
