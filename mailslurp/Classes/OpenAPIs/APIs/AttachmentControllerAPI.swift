...
@objc open class AttachmentControllerAPI : NSObject {
     ...
     */
    open class func getAttachments( fileNameFilter: String? = nil,  page: Int? = nil,  size: Int? = nil,  sort: Sort_getAttachments? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<PageAttachmentEntity> {
        let deferred = Promise<PageAttachmentEntity>.pending()
        getAttachmentsWithRequestBuilder(fileNameFilter: fileNameFilter, page: page, size: size, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
     ...
     */
    open class func getAttachmentsWithRequestBuilder(fileNameFilter: String? = nil, page: Int? = nil, size: Int? = nil, sort: Sort_getAttachments? = nil) -> RequestBuilder<PageAttachmentEntity> {
        let path = "/attachments"
        let URLString = mailslurpAPI.basePath + path
        let parameters: [String: Any]? = nil
