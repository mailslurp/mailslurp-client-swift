...
@objc open class AttachmentControllerAPI : NSObject {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getAttachments(page: Int? = nil, size: Int? = nil, sort: Sort_getAttachments? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: PageAttachmentEntity?,_ error: Error?) -> Void)) {
        getAttachmentsWithRequestBuilder(page: page, size: size, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
     ...
     */
    open class func getAttachmentsWithRequestBuilder(page: Int? = nil, size: Int? = nil, sort: Sort_getAttachments? = nil) -> RequestBuilder<PageAttachmentEntity> {
