...
@objc open class InboxForwarderControllerAPI : NSObject {
     ...
     */
    open class func getInboxForwarders( before: Date? = nil,  inboxId: UUID? = nil,  page: Int? = nil,  searchFilter: String? = nil,  since: Date? = nil,  size: Int? = nil,  sort: Sort_getInboxForwarders? = nil, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue) -> Promise<PageInboxForwarderDto> {
        let deferred = Promise<PageInboxForwarderDto>.pending()
        getInboxForwardersWithRequestBuilder(before: before, inboxId: inboxId, page: page, searchFilter: searchFilter, since: since, size: size, sort: sort).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body!)
            case let .failure(error):
     ...
     */
    open class func getInboxForwardersWithRequestBuilder(before: Date? = nil, inboxId: UUID? = nil, page: Int? = nil, searchFilter: String? = nil, since: Date? = nil, size: Int? = nil, sort: Sort_getInboxForwarders? = nil) -> RequestBuilder<PageInboxForwarderDto> {
        let path = "/forwarders"
        let URLString = mailslurpAPI.basePath + path
        let parameters: [String: Any]? = nil
