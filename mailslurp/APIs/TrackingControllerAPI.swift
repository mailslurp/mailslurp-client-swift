...
open class TrackingControllerAPI {
    /**
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getTrackingPixel(id: UUID, apiResponseQueue: DispatchQueue = mailslurpAPI.apiResponseQueue, completion: @escaping ((_ data: TrackingPixelDto?, _ error: Error?) -> Void)) {
        getTrackingPixelWithRequestBuilder(id: id).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<TrackingPixelDto> 
     */
    open class func getTrackingPixelWithRequestBuilder(id: UUID) -> RequestBuilder<TrackingPixelDto> {
