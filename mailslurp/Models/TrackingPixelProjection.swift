//
// TrackingPixelProjection.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

/** Tracking pixel data */
@objcMembers public class TrackingPixelProjection: NSObject, Codable, ParameterConvertible {

    public var userId: UUID
    public var sentEmailId: UUID?
    public var inboxId: UUID?
    public var createdAt: Date
    public var recipient: String?
    public var seen: Bool
    public var seenAt: Date?
    public var name: String?
    public var _id: UUID

    public init(userId: UUID, sentEmailId: UUID? = nil, inboxId: UUID? = nil, createdAt: Date, recipient: String? = nil, seen: Bool, seenAt: Date? = nil, name: String? = nil, _id: UUID) {
        self.userId = userId
        self.sentEmailId = sentEmailId
        self.inboxId = inboxId
        self.createdAt = createdAt
        self.recipient = recipient
        self.seen = seen
        self.seenAt = seenAt
        self.name = name
        self._id = _id
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case userId
        case sentEmailId
        case inboxId
        case createdAt
        case recipient
        case seen
        case seenAt
        case name
        case _id = "id"
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(userId, forKey: .userId)
        try container.encodeIfPresent(sentEmailId, forKey: .sentEmailId)
        try container.encodeIfPresent(inboxId, forKey: .inboxId)
        try container.encode(createdAt, forKey: .createdAt)
        try container.encodeIfPresent(recipient, forKey: .recipient)
        try container.encode(seen, forKey: .seen)
        try container.encodeIfPresent(seenAt, forKey: .seenAt)
        try container.encodeIfPresent(name, forKey: .name)
        try container.encode(_id, forKey: ._id)
    }
}


@available(iOS 13, tvOS 13, watchOS 6, macOS 10.15, *)
extension TrackingPixelProjection: Identifiable {}
